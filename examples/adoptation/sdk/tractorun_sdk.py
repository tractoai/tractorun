...
def main(toolbox: Toolbox):
    ...
    dataset2 = YtTensorDataset(toolbox=toolbox, path="//home/samples/train", columns=["data", "labels"])
    ...
    if args.save_model:
        ts = TensorSerializer()
        toolbox.save_model(ts.serialize(model.state_dict()), "//home/samples/train", metadata={})


def _default_training_dir() -> str:
    import random
    import string

    rnm = "".join(random.choices(string.ascii_letters + string.digits, k=8))
    return f"//tmp/tractorun_examples/adoptation/cli/{rnm}"


run(
    main,
    backend=Tractorch(),
    yt_path=_default_training_dir(),
