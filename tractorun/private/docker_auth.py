...
class DockerAuthInternal(TypedDict):
    username: str | None
    password: str | None
    auth: str | None
...
class DockerAuthDataExtractor:
    ...
    def _is_raw_auth_data_valid(self, raw_auth_data: Any) -> bool:
        if not isinstance(raw_auth_data, dict):
            return False
        if set(raw_auth_data.keys()) != {"username", "password"} or set(raw_auth_data.keys()) != {"auth_data"}:
            return False
    ...
    def extract(self, auth_data: DockerAuthData) -> DockerAuthInternal:
        match auth_data:
            case DockerAuthPlainText():
                assert isinstance(auth_data, DockerAuthPlainText)  # skip type warning in pycharm
                return dict(
